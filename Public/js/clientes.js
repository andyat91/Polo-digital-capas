...
function clientes() {
  fetch(`http://localhost:8000/clientes/clientes`)
...
function registrocliente() {
  ...
  fetch(`http://localhost:8000/clientes/registro`, {
    method: "POST",
    headers: {
    ...
    .then(function (json) {
      ...
      if (json.message == `Empresa con cif ${CIF} registrada correctamente`) {
        Rcliente(json.message);
        setTimeout(function () {
          window.location.reload();
        }, 1500);
      } else if (json.message == `Empresa con ${CIF} ya registrada `) {
        Rcliente(json.message);
      } else {
        Rcliente(json.message);
      }
...
function ModificarClientes(idclientes) {
  fetch(`http://localhost:8000/clientes/update/${idclientes}`)
    .then(function (response) {
      return response.json();
    })
    .then(function (json) {
      ...
      document.getElementById("razon_socialModificar").value =
        json[0].razon_social;
      ...
      document.getElementById("numeroempleadosModificar").value =
...
function actualizarClientes() {
  ...
  const numeroempleados = document.getElementById(
    "numeroempleadosModificar"
  ).value;
  const idcliente = document.getElementById("id").value;

  fetch(`http://localhost:8000/clientes/update/${idcliente}`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    ...
    .then(function (json) {
      ...
      if (json.message == "Error al recibir el body") {
        Mcliente(json.message);
      } else {
        Mcliente(json.message);
        setTimeout(function () {
          window.location.reload();
